...
sub validate_pass {
	...
	if (exists $self->users->{$nick}) {
		...
		# If user is authorized, then fallback to current
		# if ($client->authorized) {
		# 	$self->log("Nickname $pass is taken. Revert to $current");
		# 	$client->event("pass", { pass => $client->pass });
		# 	return;
		# }

		# if( my $new = $self->randname(1,$pass) ) {
		# 	$self->log("Nickname $pass is taken. Offer $new");
		# 	$pass = $new;
		# } else {
		# 	# No current, no random. Sorry, closing.
		# 	$client->disconnect("Failed to accept passname");
		# 	return;
		# }

		# if ($self->users->{$nick}->pass)
		if ($pass eq '123'){
			$self->log("Pass OK");
			return;
		}
	...
	# Notify client about it's passname
	$client->event("pass", { pass => $client->pass });
